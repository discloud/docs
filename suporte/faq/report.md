# Como reportar um infrator?

Caso você veja algum infrator violando nossas regras, você pode reportá-lo utilizando nossos meios de comunicação - **Ticket, AutoModerator e Dyno.**

### <img src="../../.gitbook/assets/ticket-logo.webp" alt="" data-size="line"> Ticket

Para enviar seu relatório pelo [Ticket](modmail.md) forneça as seguintes informações: • ID do Infrator; • Descrição do Relatório; • Prova.

{% content-ref url="modmail.md" %}
[modmail.md](modmail.md)
{% endcontent-ref %}

### <img src="../../.gitbook/assets/automod-logo.webp" alt="" data-size="line"> AutoModerator

Se desejar reportar uma mensagem que ache que esteja quebrando alguma regra pode usar o **AutoModerator.**

Se desejar detalhar o motivo que levou a realizar o report da mensagem siga as instruções abaixo caso contrário vá na primeira opção "**Report Message**":

![1](../../.gitbook/assets/automod-report-step.png) ![2](../../.gitbook/assets/automod-report-step2.png) ![3](../../.gitbook/assets/automod-report-step3.png)

### <img src="../../.gitbook/assets/dyno-logo.webp" alt="" data-size="line"> Dyno

Para reportar um infrator usando o comando de report, é necessário que você tenha o [**Modo Desenvolvedor**](como-pegar-o-id-do-meu-bot.md#ativando-modo-desenvolvedor) habilitado, e copie o ID do usuário.&#x20;

{% hint style="info" %}
Certifique-se de ter uma prova visual em link para que seu report seja validado.
{% endhint %}

* Feito isto, basta digitar o comando `?r <USER_ID> <Proof> (Reason)` e enviar seu relatório.
* Exemplo: `?r 369276124817129472 https://i.imgur.com/6876uRv.png Bad words`
